...
class Credits(object):
    ...
    def _update(self, seconds):
        # normal scrolling
        if self.scroll_root > self.screen.get_height() - self.height - 100:
    ...
    def _redraw(self):
        ...
        self.screen.blit(self.credits_bg, (self.bg_x_pos, 0))

        # blit the continuation of the background image
        if self.bg_x_pos < - (self.credits_bg.get_width() -
                              self.screen.get_width()):

            self.screen.blit(self.credits_bg, (self.bg_x_pos +
